...
{
	devices_software_iic_single_write_nd(BH1750_ADDR, BH1750_PWON);		//通电
	devices_software_iic_single_write_nd(BH1750_ADDR, BH1750_RESET);		//复位
	devices_software_iic_single_write_nd(BH1750_ADDR, BH1750_CMT_H);	
	devices_software_iic_single_write_nd(BH1750_ADDR, BH1750_CMT_L);			//设置透光率为100%
	devices_software_iic_single_write_nd(BH1750_ADDR, BH1750_H_RE_MOD2);	//设置为高精度模式
}	
/*******************************
函数名：devices_bh1750_init
函数功能：初始化BH1750
...
unsigned char devices_bh1750_measure(float* Result)
{
	...
	u16 ST = 0;
	if(devices_software_iic_single_read_nd(BH1750_ADDR, 2, temp))
	 {
		ST = (temp[0]<<8)|temp[1];
		*Result = (u16)ST / 0.6;
	 }
