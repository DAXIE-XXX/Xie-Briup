...
bool MQTTClientManager::connectToBroker(const QString &url)
{
    // 如果URL为空，使用默认配置
    if (url.isEmpty()) {
        return false;
    ...
    mqttClient->setClientId(brokerUrl.userName() + "_Desktop");
...
void MQTTClientManager::publishValue2(const QString &topic, const QString &value, int qos, bool retain)
{
    publishValue(topic, value, qos, retain);
}
